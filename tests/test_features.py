...
def test_spatial_wrist_and_foot_distance(fixture_data_dir: Path):
    ...
    assert a1.features["spatial.wrist_min_dist_norm"] == pytest.approx(0.0)
    assert a1.features["spatial.foot_min_dist_norm"] == pytest.approx(1.0)
    assert a1.features["spatial.foot_inside"] == pytest.approx(0.0)
...
def test_temporal_consecutive_hit_and_hand_move(fixture_data_dir: Path):
    ...
    assert feat6.features["temporal.hand_move_1"] >= 0.0
    ...
    per_box8 = reg.extract_per_box_features(record, frame8)
    a1 = next(pb for pb in per_box8 if pb.box_token == "S1:A1")
    assert a1.features["temporal.consecutive_hit_3"] == pytest.approx(1.0)
