...
def predict_record_with_rules(
    ...
    *,
    ...
    processor = RuleCollisionProcessor(box_index, params=params, video_fps=video_fps)
...
def _external_collision_boxes(record: RecordData) -> list[dict[str, Any]]:
    ...
    if not record.box_index:
        record.box_index = build_box_index_for_record(record)
...
def evaluate_rule_baseline(
    ...
    *,
    ...
    params: CollisionParams | None = None,
    video_fps: float = 25.0,
    model_name: str = RULE_BASELINE_NAME,
    ...
    for record in records:
        preds = predict_record_with_rules(record, params=params, video_fps=video_fps)
